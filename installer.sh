...
then
	...
	apt-get install -y libecj-java libgetopt-java libservlet3.0-java glassfish-javaee ttf-dejavu i2p i2p-router libjbigi-jni #installs i2p and other dependencies
	apt-get -f install -y # resolves anything else in a broken state
fi
...
dpkg -i /root/anonsurf/kali-anonsurf.deb || (apt-get -f install -y && dpkg -i kali-anonsurf.deb) # this will automatically install the required packages
